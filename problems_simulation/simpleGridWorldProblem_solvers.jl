...
# --------------------------simulation-----------------------------
function run_simpleGridWorld_sim(package_name, m, policy, n_simulations = 10)
    local rsum = 0
...
function run_simpleGridWorld_solvers()
    # ---------------------------solvers--------------------------------
    old_df =  DataFrame()
    ...
    for (package_name, def_solver) in solver_dict
        ...
        for i in 1:10
            ...
            local m = SimpleGridWorld(size = (3,3))
            local solver = def_solver
            local policy = solve(solver, m)
            # print(policy.alphas)
            
            local n_simulations, rsum, r_total,n_step = run_simpleGridWorld_sim(package_name, m, policy, 1000)
            
            # time taken to execute a given expression or function, in seconds
            local elapsed_time = @elapsed run_simpleGridWorld_sim(package_name, m, policy, 1000)
            # -----------------record test result--------------------------
    ...
    CSV.write(pwd()*"/results/simpleGridWorldProblem.csv", old_df)
    println("done!")
    #println(old_df)
end


run_simpleGridWorld_solvers()