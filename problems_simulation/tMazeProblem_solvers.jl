...
# --------------------------simulation-----------------------------
function run_tmaze_sim(package_name, m, policy, n_simulations = 10)
    local rsum = 0
...
function run_tmaze_solvers()
    ...
    for (package_name, def_solver) in solver_dict
        ...
        for i in 1:10
            ...
            local n_simulations, rsum, r_total,n_step = run_tmaze_sim(package_name, m, policy, 1000)
            
            # time taken to execute a given expression or function, in seconds
            local elapsed_time = @elapsed run_tmaze_sim(package_name, m, policy, 1000)
            # -----------------record test result--------------------------
